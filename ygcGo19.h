/*
* ygcGo19.h
* default rules header file
*/

#pragma once

#include "ygc.h"

namespace ygc {

	namespace Go19 {

		private ref class Go19StoneColor sealed : public ygcStoneColor { 
		public:
		
			const static uint16_t EMPTY = 0;
			const static uint16_t BLACK = 1;
			const static uint16_t WHITE = 2;

			Go19StoneColor() { stoneColor = EMPTY; };
			Go19StoneColor(uint16_t sc) { stoneColor = sc % 3; };

			inline void operator=(const Go19StoneColor %g) { this->stoneColor = g.stoneColor; }
			inline void operator=(const int &i) { this->stoneColor = i % 3; }
			inline bool operator==(const int &i) { if (i == stoneColor) return true; return false; }
		};

		const uint16_t Go19PlayerCount = 2;

		const uint16_t Go19BoardHeight = 19;
		const uint16_t Go19BoardWidth = 19;

		bool bGo19_IsEmpty(ygcBoard^, uint16_t, uint16_t);
		bool bGo19_HasLiberties(ygcBoard^, uint16_t, uint16_t);
		void vGo19_SearchForPrisoners(ygcBoard^, uint16_t, uint16_t);

		moveValidator defaultMoveValidators[3] = { bGo19_IsEmpty, bGo19_HasLiberties, NULL };
		postMoveAction defaultPostMoveActions[2] = { vGo19_SearchForPrisoners, NULL };

	};

};